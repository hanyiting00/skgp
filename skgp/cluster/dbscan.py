...
from skgp.cluster.base import elu_distance
...
class DBSCAN(object):
    ...
    def train(self, X):
        """输入数据，完成 KMeans 聚类
        :param X: list of tuple  输入数据特征，[n_samples, n_features]，如：[[0.36, 0.37], [0.483, 0.312]]
        :return: OrderedDict
        """
        # 确定数据集中的全部核心对象集合
        ...
        while len(cores):
            ...
            while len(core_deque):
                ...
                if len(coreq_neighborhood) >= self.min_pts:
                    interaction = not_visit & set(coreq_neighborhood)
                    core_deque += list(interaction)
                    not_visit = not_visit - interaction
