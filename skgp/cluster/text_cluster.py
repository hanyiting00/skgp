...
class TextCluster:
    ...
    """
    def text_cluster(self, docs, features_method='tfidf', method="dbscan", n_clusters=3, max_iter=100, eps=0.5, min_samples=2, tokenizer=list):
...
if __name__ == '__main__':
    ...
    # textCluster.text_cluster(docs, method="k-means")
    # textCluster.text_cluster(docs, features_method="count", method="dbscan", eps=0.4, min_samples=20)

    corpus = ["判断unicode是否是汉字。", "判断unicode是否是汉字2。", "全角符号转半角符号。", "一些基于自然语言处理的预处理过程也会在本文中出现。"]
    # corpus = ["判断unicode是否是汉字。"]
    clusters = textCluster.text_cluster(corpus, features_method="count", method="k-means", n_clusters=3)
    # clusters = textCluster.text_cluster(corpus, features_method="count", method="dbscan", eps=0.4, min_samples=20)
    print(clusters)
